...
namespace RecruitmentServer.Models
{
	...
	{// Клас для зберігання інформації про бали
		...
		internal EducationDegreePoint[] Degrees { get; set; }// Бали за певні ступені освіти

		internal Points() { }

		internal void Change(int ageUnder18, int age18_30, int age30_50, int ageOver50, int expNone, int expUnderYear, int exp1_3, int expOver3, int diploma, int noChronicDiseases, int driverLicense, int noSmoker, int noDrinkAlcohol, int businessTripOpportunity, EducationDegreePoint[] degrees)
		{// Метод, який змінює дані
			...
			Degrees = new EducationDegreePoint[degrees.Length];
